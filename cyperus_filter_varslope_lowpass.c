...
void cyperus_filter_varslope_lowpass_init(struct cyperus_parameters *filter, int jack_sr) {
  ...
  cyperus_lo_designVarSlopeFilter(filter->slope, filter->fs, filter->B, filter->A);
  
} /* cyperus_filter_varslope_lowpass_init */

void cyperus_filter_varslope_lowpass(struct cyperus_parameters *filter, int samplerate, int pos) {
  ...
  for (ioIdx = 0; ioIdx < 8; ioIdx++) {
    filter->W0_FILT_STATES[ioIdx] = 0.0f;
  }
